...
function toRefs(object) {
	if (!isProxy(object)) warn$2(`toRefs() expects a reactive object but received a plain one.`);
  ...
  - expected on client: ${clientText}`);
			...
			if (props) {
				...
				for (const key in props) {
					...
					if (forcePatch && (key.endsWith("value") || key === "indeterminate") || isOn(key) && !isReservedProp(key) || key[0] === "." || isCustomElement) patchProp$1(el, key, null, props[key], void 0, parentComponent);
				}
			}
			let vnodeHooks;
